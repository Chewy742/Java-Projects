...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // TODO
        double x = (ux-vx)*(ux-vx);
        double y = (uy-vy)*(uy-vy);
        
        return Math.sqrt(x+y); // Replace this
  }
  ...
  public void computeAllEuclideanDistances() {
      ...
      while (itr.hasNext()){
          ...
          List<Edge> edges = current.adjacentEdges;  
          Iterator<Edge> itr2 = edges.iterator();
          while (itr2.hasNext()){
              Edge currentEdge = itr2.next();
              currentEdge.distance = computeEuclideanDistance(currentEdge.source.x, currentEdge.source.y, currentEdge.target.x, currentEdge.target.y);
          }
      }
  ...
  public void doDijkstra(String s) {
      ...
      // set every vertice distance to infinity
      Collection<Vertex> allVertices = getVertices();
      Iterator<Vertex> itr = allVertices.iterator();
      ...
      //set the vertex you are at to dist = 0
      Vertex start = getVertex(s);
      start.distance = 0.0;
      boolean stop = ifTrueStop();
      
      //iterating through the vertices
      while (!stop){
          
          Vertex v = getLowestVertex(myList); //get the Lowest one
          v.known = true;
          myList.remove(v);
          
          List<Edge> adjacents = v.adjacentEdges;
          Iterator<Edge> edgeitr = adjacents.iterator();
          while (edgeitr.hasNext()) {
              Edge current = edgeitr.next();
              Vertex adj = current.target; //one of the adjacent vertexes 
              if(adj.known == false){ //if its not already known 
                  double toAdd = current.distance;
                  //check if the distance to it is less than whatever is already there
                  if((v.distance + toAdd) < adj.distance){
                      adj.distance = v.distance + toAdd;
                      adj.prev = v;                      
                  }
              }
          }
  ...
  private Vertex getLowestVertex(ArrayList<Vertex> l){
      Vertex min = l.get(0);
      for (int i = 0; i < l.size(); i++){
          Vertex current = l.get(i);
  ...
  private boolean ifTrueStop(){ //if there is a vertex with a false known field return false
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    boolean stop = false;
    Vertex last = getVertex(t);
    
    while (!stop){
        Vertex prev = last.prev;
        double distancebetween = last.distance - prev.distance;
        Edge toAdd = new Edge(prev, last, distancebetween);    
        edgeList.add(0, toAdd);
        last = prev;
        if(prev.name.equals(s)){
            stop = true;
        }
    }    
      
    clear();
    return edgeList;  //Replace this
  }
