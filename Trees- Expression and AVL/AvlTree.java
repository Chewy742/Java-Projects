...
public class AvlTree<AnyType extends Comparable<? super AnyType>>
{
    ...
    //MY ADDITION
    public List getLineList( AnyType x )
    {
        return getLineList( x, root );
    }
    private List getLineList( AnyType x, AvlNode<AnyType> t ){
        while( t != null )
        {
            int compareResult = x.compareTo( t.element );
    ...
    private void printIndex( AvlNode<AnyType> t )
    {
        if( t != null )
        {
            printIndex( t.left );
            String toprint = t.element + ": ";
            LinkedList<Integer> printList = t.numList;
            Iterator<Integer> itr = printList.iterator();
            while(itr.hasNext()){
                toprint += itr.next();
                if (itr.hasNext()){
                    toprint += ", ";
    ...
    {
        if( t == null )
            return new AvlNode<>( x, null, null, linenum );
        ...
        if( compareResult < 0 ){
            ...
            if(t.numList.contains(linenum)){
            
            }else{
                t.numList.add(linenum);
            }
